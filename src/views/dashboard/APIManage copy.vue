...
const elements = ref([
  { id: '1', type: 'custom', data: { type: 'custom' }, label: 'API管理', position: { x: 0, y: 0 }, class: 'light', sourcePosition: Position.Right, targetPosition: Position.Left },
] as any);
onMounted(async () => {
  await nextTick();
  ...
  for(let i = 0; i < originData.value.length; i++) {
    elements.value.push({
      id: `${originData.value[i].name}`, label: `${originData.value[i].name}`, position: { x: 200, y: i * 100 }, class: 'light', sourcePosition: Position.Right, targetPosition: Position.Left 
    });

    // elements.value.push( { id: 'el' + `${originData.value[i].name}`, source: '1', sourceHandle: '0', target: `${originData.value[i].name}`, animated: true });

    for (let j = 0; j < originData.value[i].data.api.length; j++) {
      elements.value.push({
      id: `${originData.value[i].data.api[j].name}`, label: `${originData.value[i].data.api[j].name}`, position: { x: 400, y: j * 100 }, class: 'light', sourcePosition: Position.Right, targetPosition: Position.Left 
    });
    }
  }
});
